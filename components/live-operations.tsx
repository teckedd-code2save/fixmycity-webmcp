...
export function LiveOperationsMetrics(){
 ...
 return <><div className="operations-kpis"><div><strong>{health.reports.open}</strong><span>Open reports</span></div><div><strong className="text-[var(--signal-red)]">{health.reports.critical}</strong><span>Critical now</span></div><div><strong>{health.reports.resolved}</strong><span>Resolved</span></div><div><strong>{health.activeAssignments}</strong><span>Field assignments</span></div></div><p className="workspace-source">Seeded challenge workspace · every new report and status change persists in D1</p></>;
}
