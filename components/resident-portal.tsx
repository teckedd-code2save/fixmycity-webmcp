...
export function ResidentPortal() {
  ...
  async function submit(event:SyntheticEvent<HTMLFormElement,SubmitEvent>) {
    ...
    const payload={title:form.get('title'),description:form.get('description'),category:form.get('category'),severity:form.get('severity'),address:form.get('address'),landmark:form.get('landmark'),affectedPeople:Number(form.get('affectedPeople')??1),...point,imageKey};
    const response=await fetch('/api/reports',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json() as {error?:string;report:{id:string}};setSubmitting(false);
    if(!response.ok){alert(result.error);return;} setSuccess(result.report.id); event.currentTarget.reset(); const updated=await fetch('/api/reports').then(async(r)=>await r.json() as {reports:Report[]});setReports(updated.reports?.slice(0,4)??[]);
  }
  async function confirm(id:string){await fetch(`/api/reports/${id}/confirm`,{method:'POST'});const updated=await fetch('/api/reports').then(async(r)=>await r.json() as {reports:Report[]});setReports(updated.reports?.slice(0,4)??[]);}
  return <main className="portal-page">
    <header className="portal-header"><Link href="/" className="portal-brand"><span><MapPin/></span><strong>FixMyCity</strong></Link><Link href="/" className="back-link"><ArrowLeft/> Operations center</Link></header>
    <section className="resident-hero"><div><div className="eyebrow"><Radio/> Resident reporting</div><h1>Spot it. Pin it.<br/><em>Help fix it.</em></h1><p>Report a local problem in under two minutes. Your evidence joins nearby signals so city teams can respond with context.</p></div><div className="trust-card"><ShieldCheck/><div><strong>Your report stays traceable</strong><span>Every status change is recorded and visible to residents.</span></div></div></section>
    <div className="resident-layout">
      <form className="report-form" onSubmit={submit}>
        <div className="form-heading"><span>01</span><div><h2>What happened?</h2><p>Share observable facts. Avoid personal or sensitive information.</p></div></div>
        ...
        <Button type="submit" disabled={submitting} className="submit-report">{submitting?'Submitting evidence…':<>Submit civic report <Send/></>}</Button>
      </form>
      <aside className="nearby-panel"><div className="section-title-row"><div><h2>Nearby signals</h2><p>Confirm an existing report instead of duplicating it</p></div></div>{reports.map((report)=><article key={report.id} className="nearby-card"><div><span className={`severity-dot ${report.severity}`}/><strong>{report.title}</strong></div><p>{report.address}</p><footer><span>{report.confirmations} confirmations · {report.status}</span><button onClick={()=>confirm(report.id)}>I see this too</button></footer></article>)}</aside>
    </div>
  </main>;
}
