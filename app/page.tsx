...
import { LiveIncidentFeed, LiveOperationsMetrics } from '@/components/live-operations';
    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="app-header">
        <div className="flex items-center gap-3">
          ...
          <div><p className="brand-name">FixMyCity</p><p className="brand-kicker">Operations workspace</p></div>
        </div>
        <nav className="actor-switch" aria-label="Actor workspaces"><span>Operations</span><Link href="/resident">Resident</Link><Link href="/inspector">Inspector</Link></nav>
        <div className="flex items-center gap-2">
          <div className="sync-pill"><span /> D1 connected</div>
          <Link href="/inspector" className={buttonVariants({variant:'outline',className:'h-10 rounded-full px-4'})}><Users data-icon="inline-start" /> Field view</Link>
