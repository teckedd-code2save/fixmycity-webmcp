...
export async function POST(request: Request) {
  ...
  const inspectorId = body.inspectorId ?? 'INS-001'; const route = JSON.parse(proposal.payload) as { routeId:string; stops:Array<{id:string}> }; const now=Date.now(); const db=database();
  await db.batch([
