...
const retiredSampleIds = ['FC-0241','FC-0238','FC-0234','FC-0237','FC-0229','FC-0225','FC-0218','FC-0211'];
...
export async function ensureDatabase() {
  ready ??= (async () => {
    ...
    const now = Date.now();
    await db.batch([
      db.prepare(`INSERT OR IGNORE INTO profiles (id,email,name,role,created_at) VALUES (?,?,?,?,?)`).bind('public-resident','resident@fixmycity.local','Resident submission','resident',now),
      db.prepare(`INSERT OR IGNORE INTO profiles (id,email,name,role,created_at) VALUES (?,?,?,?,?)`).bind('operations-user','operations@fixmycity.local','Operations team','operator',now),
      db.prepare(`INSERT OR IGNORE INTO profiles (id,email,name,role,created_at) VALUES (?,?,?,?,?)`).bind('field-team-user','field@fixmycity.local','Field team','inspector',now),
      db.prepare(`INSERT OR IGNORE INTO inspectors (id,profile_id,phone,transport,availability,start_latitude,start_longitude) VALUES (?,?,?,?,?,?,?)`).bind('FIELD-TEAM-01','field-team-user',null,'motorbike','available',5.5652,-0.1931),
    ]);
    const cleaned = await db.prepare(`SELECT value FROM app_meta WHERE key = 'retired_sample_reports'`).first<{value:string}>();
    if (!cleaned) {
      const placeholders=retiredSampleIds.map(()=>'?').join(',');
      await db.batch([
        db.prepare(`DELETE FROM assignments WHERE report_id IN (${placeholders})`).bind(...retiredSampleIds),
        db.prepare(`DELETE FROM updates WHERE report_id IN (${placeholders})`).bind(...retiredSampleIds),
        db.prepare(`DELETE FROM proposals WHERE ${retiredSampleIds.map(()=>`payload LIKE ?`).join(' OR ')}`).bind(...retiredSampleIds.map(id=>`%${id}%`)),
        db.prepare(`DELETE FROM reports WHERE id IN (${placeholders})`).bind(...retiredSampleIds),
        db.prepare(`INSERT INTO app_meta (key,value) VALUES ('retired_sample_reports','1')`),
      ]);
    }
  })();
  return ready;
